...
class DBManagerRealm: NSObject{
    ...
    func addSpending(name:String, currency: String, date: Date, cycle: String, price: String){
        let spending = SpendingModel()
        spending.id = 0
        spending.name = name
        ...
        do{
            let realm = try Realm()
            var lastRecord = realm.objects(SpendingModel.self).last?.id
            if lastRecord == nil{
                lastRecord = 0
            }else{
                lastRecord! += 1
            }
            spending.id = lastRecord!
            try realm.write{
                realm.add(spending)
            }
        }catch{
    ...
    func getSpendings() -> Results<SpendingModel>{
        var rec: Results<SpendingModel>?
        do{
            let realm = try Realm()
            let record = realm.objects(SpendingModel.self)
            rec = record
        }catch{
